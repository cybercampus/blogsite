...
class Tag(models.Model):
    name = models.CharField(max_length=21, verbose_name='文章标签')
    add_date = models.DateTimeField(auto_now_add=True, verbose_name='添加时间')
    pub_date = models.DateTimeField( auto_now=True, verbose_name='修改时间')
...
class Sidebar(models.Model):
    ...
    content = models.CharField(max_length=500,blank=True,default='', verbose_name="内容", help_text="如果设置的不是 HTML 类型，可为空")
    sort = models.PositiveIntegerField(default=1, verbose_name="排序", help_text="序号越打越靠前")
    ...
    @classmethod    #类方法装饰器，在这个就变成了类的一个方法，可以调用
    def get_sidebar(cls):
        return cls.objects.filter(status=2) # 查询所有允许展示的模块
    ...
    def get_content(self):
        if self.display_type == 1:  #搜索
            ...
            return render_to_string('blog/sidebar/comment_post.html',context=context)
        elif self.display_type == 5:
            context = {

            }
            return render_to_string('blog/sidebar/archives_post.html',context=context)
